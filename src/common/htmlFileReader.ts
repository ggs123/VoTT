...
import { IAsset, AssetType, IFileInfo, IRegion } from "../models/applicationState";
...
export default class HtmlFileReader {
    ...
    //new*: return each region blob 
    public static async getRegionBlob(asset: IAsset, region: IRegion): Promise<Blob> {
        Guard.null(asset);
        ...
        let data = null;
        if (asset.type === AssetType.VideoFrame) {
            data = await this.getRegionImage(asset,region);
        } else {
            // Download the asset binary from the storage provider
            const response = await axios.get<Blob>(asset.path, config);
            if (response.status !== 200) {
    ...
    //new*: return each region blob buffer
    public static async getRegionArray(asset: IAsset, region: IRegion): Promise<ArrayBuffer> {
        const blob = await this.getRegionBlob(asset,region);
        return await new Response(blob).arrayBuffer();
    }
    ...
    //new*: return each region picture blob
    public static async getRegionImage(asset: IAsset, region: IRegion): Promise<Blob> {
        return new Promise<Blob>((resolve, reject) => {
            const cachingEnabled = false;
            let refresh = !cachingEnabled;
            let video: HTMLVideoElement = this.videoAssetFiles[asset.parent.name];
